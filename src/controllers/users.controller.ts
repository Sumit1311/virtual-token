...
export default class UserController {
    ...
    static async renewLogin(req: Request, res: Response) {
        let response;
        try {
            response = ResponseBuilder.getDefaultResponse();
            const body = new RenewTokenDTO(req.body);
            response.setBody(await UserController.userService.renewLogin(body));
            response.setStatus(HttpStatus.OK);
        } catch (error) {
            response = handleError(error.message);
            console.log(error);
